...
Реализуй небольшой `TransferFactory`, который создаёт завершённый перевод. В этом задании считай, что команда уже
прошла бизнес-проверки, а балансы и ledger entries уже успешно изменены. Фабрика отвечает только за создание объекта
`Transfer`; проверку достаточности средств, статусов счетов, валюты и атомарность операции здесь реализовывать не нужно.

Передавай источники идентификаторов и времени в конструктор фабрики:
...
Для входных данных можешь объявить отдельный неизменяемый объект `CreateTransferCommand` со следующими полями:
...
Метод фабрики должен иметь контракт, эквивалентный следующему:

```java
Transfer create(CreateTransferCommand command);
```

При каждом вызове `create` фабрика должна:

1. ровно один раз вызвать `idGenerator.get()` и использовать результат как `Transfer.id`;
2. получить время только через переданный `Clock`, а не через `Instant.now()` без аргумента и не через
   `System.currentTimeMillis()`;
3. перенести в `Transfer` все данные команды без случайного изменения суммы, валюты или описания;
4. установить статус `COMPLETED`, потому что по техническим требованиям объект перевода создаётся только после успешной
   синхронной операции;
5. установить `createdAt` и `completedAt`. Для этой упрощённой фабрики получи один `Instant` и запиши его в оба поля:
   создание и завершение считаются одним доменным событием. Это также не позволит двум последовательным обращениям к
   изменяемым часам случайно сформировать разные значения.

Подготовь два варианта создания фабрики:

- рабочий вариант с `UUID::randomUUID` и `Clock.systemUTC()`;
- тестовый вариант с управляемым `Supplier<UUID>` и `Clock.fixed(...)`.

В unit-тестах проверь как минимум следующие сценарии.

#### Сценарий 1. Детерминированное создание перевода

Задай заранее известные UUID перевода и счетов, а также фиксированный момент времени. После вызова `create` проверь:

- `id` равен значению, возвращённому тестовым `Supplier`;
- `sourceAccountId`, `targetAccountId`, `amount`, `currency` и `description` совпадают с командой;
- статус равен `COMPLETED`;
- `createdAt` и `completedAt` равны моменту из `Clock.fixed(...)`.

#### Сценарий 2. Новый идентификатор для каждого перевода

Передай `Supplier`, который последовательно возвращает два заранее известных UUID. Дважды вызови `create` и докажи,
что первый и второй переводы получили соответствующие идентификаторы. Дополнительно посчитай вызовы `Supplier` и
проверь, что на каждый перевод приходится ровно один вызов.

Не проверяй в тестах конкретное значение от `UUID.randomUUID()` или текущее системное время: такой тест будет
недетерминированным. Цель задания — показать, что недетерминированные значения становятся обычными зависимостями и
могут полностью контролироваться в unit-тесте.

После выполнения письменно ответь на два вопроса:

- почему для времени используется специализированный `Clock`, а для UUID в этом небольшом примере достаточно
  `Supplier<UUID>`;
- в какой момент `Supplier<UUID>` стало бы полезно заменить собственным интерфейсом `IdGenerator`.
